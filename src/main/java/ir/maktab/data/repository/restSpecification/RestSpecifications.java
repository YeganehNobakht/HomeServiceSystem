package ir.maktab.data.repository.restSpecification;
...
import ir.maktab.dto.restDto.OrderFilterDto;
import ir.maktab.dto.restDto.SuggestionFilterDto;
import ir.maktab.dto.restDto.UserReportDto;
...
public interface RestSpecifications {
    ...
    static Specification<CustomerOrder> filterOrders(OrderFilterDto dto){
        return (Specification<CustomerOrder>) (root, query, criteriaBuilder) -> {
            ...
            if (!StringUtils.isNullOrEmpty(dto.getSubServiceName())) {
                predicateList.add(criteriaBuilder.equal(subCategoryJoin.get("name"), dto.getSubServiceName()));
            }
            if (!StringUtils.isNullOrEmpty(dto.getServiceName())) {
